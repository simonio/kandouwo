...
function sort_files_asc($left, $right)
{
  return strcmp($left->datetime, $right->datetime);
}

function sort_files_desc($left, $right)
{
  return strcmp($left->datetime, $right->datetime) * -1;
}
...
class UploadHandler
{
  ...
  function __construct($dir = '/files/', $options = null, $initialize = true, $error_messages = null)
  {
    $this->options = array(
  ...
  protected function get_desc_file_path($filename)
  {
    ...
    if (!is_dir($file_dir)) {
      mkdir($file_dir, $this->options['mkdir_mode'], true);
    }
    $desc_file = $file_dir . '/' . $desc_file_name . '.txt';
    return $desc_file;
  }
  ...
  {
    // 添加文件描述
    //$file->desc = '文件 '.$file->name.' 的描述。';
    $desc_file = $this->get_desc_file_path($file->name);
    if (is_file($desc_file)) {
      $file->desc = file_get_contents($desc_file);
    }
    
    // 添加文件修改日期时间
    $date_time = filemtime($desc_file);
    $file->datetime = date("Y-m-d H:i:s", $date_time);
  ...
  protected function handle_file_upload($uploaded_file, $name, $desc, $size, $type, $error, $index = null, $content_range = null)
  {
    $file       = new \stdClass();
      ...
      if ($uploaded_file && is_uploaded_file($uploaded_file)) {
        ...
        if ($append_file) {
          ...
          if (isset($desc)) {
            ...
            file_put_contents($desc_file, '添加描述吧，亲~');
          }
        }
      } else {
  ...
  public function get_file_param_name()
  {
    return $this->options['param_name'];
  }
  
  public function get($print_response = true, $sort='desc')
  {
    ...
    if ($file_name) {
      ...
      if ($sort === 'asc') {
        ...
        usort($response[$this->options['param_name']], 'sort_files_desc');
      }
    }
